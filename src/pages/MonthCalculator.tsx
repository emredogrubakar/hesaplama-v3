...
import { calculateMonthsBetween, type MonthCalculationResult } from '../utils/monthCalculations';
import { generateMonthWhatsAppMessage } from '../utils/monthShareHelpers';
...
export function MonthCalculator() {
  ...
  const [result, setResult] = useState<MonthCalculationResult | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateDates(startDate, endDate)) {
      const calculationResult = calculateMonthsBetween(
        parseDate(startDate),
        parseDate(endDate)
      );
  ...
  const handleWhatsAppShare = () => {
    if (result && startDate && endDate) {
      const message = generateMonthWhatsAppMessage({
        ...result,
        startDate,
        endDate
      });
    ...
    <main className="max-w-2xl mx-auto px-4 py-8 sm:py-12">
      <header className="text-center mb-8 sm:mb-12">
        ...
        <p className="text-gray-600">
          İki tarih arasındaki ay sayısını detaylı olarak hesaplayın
        </p>
      </header>
              ...
              <div className="bg-indigo-50 rounded-lg p-6">
                <div className="text-center">
                  ...
                  <p className="text-3xl font-semibold text-indigo-900">
                    {result.totalMonths.toLocaleString('tr-TR')} ay
                  </p>
                </div>
              </div>
